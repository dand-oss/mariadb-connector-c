  ...
  while (1) {
    ...
    ********************************************************************/
    ...
    case GTID_EVENT:
    {
      ...
      /* Shift stream pointer based on the Group Commit flag bit (Bit 1 / 0x02) */
      if (rpl_event->event.gtid.flags & 2) /* FL_GROUP_COMMIT_ID */
      {
        /* Group Commit: Read the 8-byte commit_id and step past 21 bytes total */
        RPL_CHECK_POS(ev, ev_end, post_header_len + 2);
        rpl_event->event.gtid.commit_id = uint8korr(post_header_start + 13);
        ev = post_header_start + post_header_len + 2;
      }
      else
      {
        /* Standard Fallback / XA Context: Skip the 6 reserved padding bytes */
        ev = post_header_start + post_header_len;
      }
      break;
    }
